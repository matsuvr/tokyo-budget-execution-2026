...
function breakdownBar(candidate) {
    ...
    if (spent == null || carryover == null || unused == null)
        return null;
    const segments = [
    ...
    const bar = el("div", { class: "breakdown-bar", role: "img", "aria-label": segments.map((s) => s.label).join(", ") }, ...segments.map((segment) => el("span", {
...
export function renderCandidateItem(candidate) {
    ...
    const definitionList = el("dl", { class: "candidate-grid" }, ...rows.flatMap(([term, description]) => [el("dt", {}, term), el("dd", {}, description)]));
