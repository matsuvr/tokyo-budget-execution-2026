...
import { ALL, availableValues, clearAttentionFilters } from "./attention-filter.js";
import { ATTENTION_FLAG_LABELS, GAP_COMPOSITION_LABELS, } from "./labels.js";
...
export function renderAttentionFilters(allRecords, filteredCount, filters, sort, callbacks) {
    const update = (patch, nextSort = sort) => callbacks.onChange({ ...filters, ...patch }, nextSort);
    ...
    return el("section", { class: "card filter-card", "aria-label": "明細の絞り込みと並べ替え" }, el("h2", {}, "絞り込み・並べ替え"), el("div", { class: "filter-controls" }, selectControl("局・分野（款）", "attention-bureau", filters.bureau, bureauOptions, (value) => update({ bureau: value })), selectControl("執行ギャップの内訳", "attention-composition", filters.gapComposition, compositionOptions, (value) => update({ gapComposition: value })), selectControl("2026年度予算", "attention-comparison", filters.comparison, comparisonOptions, (value) => update({ comparison: value })), selectControl("注目ポイント", "attention-signal", filters.signal, signalOptions, (value) => update({ signal: value })), selectControl("並べ替え", "attention-sort", sort, sortOptions, (value) => update({}, value))), el("p", { class: "filter-count", "aria-live": "polite" }, `${filteredCount.toLocaleString("ja-JP")} 件 / 全 ${allRecords.length.toLocaleString("ja-JP")} 件`), reset);
}
