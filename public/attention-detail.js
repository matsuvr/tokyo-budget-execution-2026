...
export function renderAttentionDetail(detail) {
    ...
    return el("div", { class: "detail-panel card" }, el("section", { class: "detail-block" }, el("h4", {}, "会計上の事実"), el("dl", { class: "compact-grid" }, el("dt", {}, "予算現額"), el("dd", {}, formatYenExact(item.amounts.currentBudgetYen)), el("dt", {}, "支出済額"), el("dd", {}, formatYenExact(item.amounts.spentYen)), el("dt", {}, "翌年度繰越額"), el("dd", {}, formatYenExact(item.amounts.carryoverYen)), el("dt", {}, "不用額"), el("dd", {}, formatYenExact(item.amounts.unusedYen)), el("dt", {}, "年度内未執行額"), el("dd", {}, formatYenExact(item.amounts.yearEndUnexecutedYen)))), el("section", { class: "detail-block" }, el("h4", {}, "構成明細"), detail.breakdown.reconciliation === "mismatch"
