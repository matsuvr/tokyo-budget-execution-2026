...
```text
年度内執行ギャップ額 = 翌年度継続分 + 年度内対応余地
年度内執行ギャップ率 = 年度内執行ギャップ額 / 予算現額
年度内対応余地 = 予算現額 - 支出済額 - 翌年度継続分
```

`年度内対応余地` は「翌年度にまだ使える残額」という意味ではありません。決算上、支出済みでも翌年度継続でもない部分を、政策・執行の追加検証につなげるための表示名に読み替えたものです。需要変動、経費節減、制度上の事情等も含み得るため、全額を執行不足とはみなしません。

内部JSONでは原本追跡のため `carryoverYen` / `unusedYen` / `yearEndUnexecutedYen` 等の既存フィールド名を維持します。表示語彙だけを政策検証の目的に合わせて変換しており、会計原値や計算式は変更していません。
...
| ファイル | 内容 |
|---|---|
| `data/normalized/execution-review/fy2024/execution-scan.json` | 全決算階層。年度内執行ギャップの内部原値とreview scope付き |
...
- `予算現額 = spentYen + carryoverYen + unusedYen`
- `yearEndUnexecutedYen = carryoverYen + unusedYen`
- itemIdの一意性
...
1. 冒頭で「2024年度の行政サービス・事業明細数」と年度内執行ギャップ額、その翌年度継続分・年度内対応余地の内訳を示す。
2. 主一覧が年度内執行ギャップ額順であり、単一の残額率だけの順位ではないことを示す。
