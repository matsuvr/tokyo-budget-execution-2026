...
export function renderAttentionItem(
  ...
  return el(
    ...
    el(
      "p",
      { class: "primary-value" },
      el("strong", {}, `年度内未執行額 ${formatYen(item.amounts.yearEndUnexecutedYen)}`),
      `（${formatRate(item.rates.yearEndUnexecutedRate)}）`,
    ),
    el("p", { class: "sub" }, "翌年度繰越額と不用額の合計"),
    breakdownBar(item),
    el(
      "p",
      { class: "exact-amounts" },
      `${formatYenExact(item.amounts.currentBudgetYen)} ＝ 支出済 ${formatYenExact(item.amounts.spentYen)} ＋ 翌年度繰越 ${formatYenExact(item.amounts.carryoverYen)} ＋ 不用 ${formatYenExact(item.amounts.unusedYen)}`,
    ),
    el(
      "div",
      { class: "badge-list", "aria-label": "確認シグナル" },
    ...
    el(
      ...
      el("dt", {}, "未執行の内訳"), el("dd", {}, gapCompositionLabel(item.gapComposition)),
