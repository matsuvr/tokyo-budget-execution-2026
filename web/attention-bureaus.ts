    ...
    ...operational.map((row) => {
      ...
      return el(
        ...
        el(
          ...
          el("dt", {}, "年度内執行ギャップ額"), el("dd", {}, `${formatYen(row.amounts.yearEndUnexecutedYen)}（${formatRate(row.rates.yearEndUnexecutedRate)}）`),
          el("dt", {}, "翌年度継続分"), el("dd", {}, formatYen(row.amounts.carryoverYen)),
          el("dt", {}, "年度内対応余地"), el("dd", {}, formatYen(row.amounts.unusedYen)),
          el("dt", {}, "2026年度予算90%以上継続"), el("dd", {}, `${(row.flagCounts["budget-continues"] ?? 0).toLocaleString("ja-JP")}件`),
          el("dt", {}, "2026年度予算増額"), el("dd", {}, `${(row.flagCounts["budget-expanded"] ?? 0).toLocaleString("ja-JP")}件`),
        ),
      );
    }),
  );
