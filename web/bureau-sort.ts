import type { BureauRowView } from "./types.js";
...
export function sortBureausForDisplay(bureaus: readonly BureauRowView[]): BureauRowView[] {
  return [...bureaus].sort(
    (a, b) =>
      b.needsExplanationCount - a.needsExplanationCount ||
      b.fy2024UnusedYen - a.fy2024UnusedYen ||
      (a.chapter < b.chapter ? -1 : a.chapter > b.chapter ? 1 : 0),
  );
}

export interface BureauTotalsVerification {
  consistent: boolean;
  mismatches: string[];
}
 ...
 */
export function verifyBureauTotals(
  bureaus: readonly BureauRowView[],
  summary: { totalComparableCount: number; totalFy2024CurrentBudgetYen: number },
): BureauTotalsVerification {
  ...
  if (comparableCount !== summary.totalComparableCount) {
    mismatches.push(
      `比較可能件数の合計（${comparableCount}）がsummary（${summary.totalComparableCount}）と一致しません`,
    );
  }
